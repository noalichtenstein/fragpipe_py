...
def multiple_run_msfragger_runner_if_not_empty(all_proteomes_folder, proteomes_fasta_list, raw_name, organism_name):
    ...
    with open(f"{all_proteomes_folder}\\{organism_name}_completed_runs.txt", 'w') as file:
        for prot in proteomes_fasta_list:
            fasta_folder = f'{all_proteomes_folder}\\{organism_name.replace("+", "_")}\\{prot}'
            work_folder = f'{all_proteomes_folder}\\{organism_name.replace("+", "_")}\\{prot}\\{raw_name}'  # folder with the name of the fasta
...
def combine_human_and_bacteria(new_proteomes_folder, proteomes_fasta_list, organism_name):
    for prot in proteomes_fasta_list:
        new_folder_name = f"{new_proteomes_folder}\\{organism_name.replace('+','_')}\\{prot}"
...
if __name__ == '__main__':
    ...
    for organism_name in organisms:
        all_proteomes_folder = rf"C:\Users\owner\Microbiome\Analyses\top20_runs"
        cur_organism_folder = f'{all_proteomes_folder}\\{organism_name.replace("+", "_")}'
        os.makedirs(cur_organism_folder)

        proteomes_fasta_list = fasta_download.download_all_fastas_of_organism_random(organism_name, cur_organism_folder, 20)
        combine_human_and_bacteria(all_proteomes_folder, proteomes_fasta_list, organism_name)

        # creating raw folders in fasta folder for each raw file
        for proteom in proteomes_fasta_list:
            for raw_name in raw_list:
                create_raw_folder(rf'{cur_organism_folder}\{proteom}', raw_name)
